...
from kekoviq.razan._islam import *
...
@kekoviq.on(events.NewMessage(pattern=r"^.اختصر (.+) (.+)"))
...
@kekoviq.on(events.NewMessage(pattern=r"^\.(.+)"))
...
@kekoviq.on(events.NewMessage(pattern=r"^.قائمه الاختصار|قائمة الاختصار"))
...
@kekoviq.on(events.NewMessage(pattern=r"^\.امسح (.+)$"))
