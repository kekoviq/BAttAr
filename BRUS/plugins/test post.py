...
async def posts(speedrunminecraft, reply, interval, mode):
    while speedrunminecraft in rioishere:
        try:
            if reply.sticker:
                await kekoviq.send_file(speedrunminecraft, reply.media)
            elif reply.reactions and reply.reactions.recent_reactions:
                for reaction in reply.reactions.recent_reactions:
                    await kekoviq(functions.messages.SendReactionRequest(
                ...
                if mode == "امن":
                    ...
                    await kekoviq.send_message(speedrunminecraft, modified_message)
                else:
                    await kekoviq.send_message(speedrunminecraft, rioishere[speedrunminecraft].message)
...
kekoviq.loop.create_task(restore_tasks())