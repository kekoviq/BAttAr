...
@kekoviq.ar_cmd(
    ...
    info={
        ...
        "examples": "{tr}رسالة @ISVSVI حي الله  ان",
    },
)
...
@kekoviq.ar_cmd(pattern="ازالة التوجيه")
async def Reda (event):
    if event.message.reply_to_msg_id:
        ...
        if replied_msg.media:
            ...
            await kekoviq.send_message(event.chat_id, "", file=replied_msg.media)
        else:
            
            await kekoviq.send_message(chat_id=event.chat_id, message=replied_msg.message)
    else:
