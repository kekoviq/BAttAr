...
async def save_media(event):
    ...
    try:
        ...
        entity = await kekoviq.get_entity(channel_username_or_id)      
        message = await kekoviq.get_messages(entity, ids=message_id)
        if not message:
    ...
    try:
        message = await kekoviq.get_messages(channel_username_or_id, ids=message_id)
        if not message:
        ...
        if message.media:
            ...
            await kekoviq.download_media(message, file=file_path)

            await kekoviq.send_file('me', file=file_path, caption=message.text)
...
async def Hussein(event):
    ...
    try:
        channel_entity = await kekoviq.get_entity(channel_username)
        messages = await kekoviq.get_messages(channel_entity, limit=limit)
    except Exception as e:
    ...
    for message in messages:
        try:
            if message.media:
                ...
                await kekoviq.send_file("me", file=file_path)
                os.remove(file_path)
