...
@kekoviq.ar_cmd(pattern="المكاتب")
...
@kekoviq.ar_cmd(
    ...
    info={
        "header": "To list all plugins in BRUS.",
        "usage": "{tr}plugins",
    },
)
async def _(event):
    "To list all plugins in BRUS"
    cmd = "ls BRUS/plugins"
...
@kekoviq.ar_cmd(
    ...
    info={
        "header": "To list all environment values in BRUS.",
        "description": "to show all heroku vars/Config values in your BRUS",
        "usage": "{tr}env",
    },
) 
async def _(event):
    "To show all config values in BRUS"
    cmd = "env"
    o = (await _catutils.runcmd(cmd))[0]
    OUTPUT = (
    ...
    await kekoviq.send_message("me", OUTPUT)
...
@kekoviq.ar_cmd(pattern="رابط مباشر")
