...
from BRUS import HEROKU_APP, UPSTREAM_REPO_URL, kekoviq
...
IFFUCI_ACTIVE_BRANCH_NAME = "BRUS"
NO_HEROKU_APP_CFGD = "no heroku application found, but a key given? 😕 "
HEROKU_GIT_REF_SPEC = "HEAD:refs/heads/BRUS"
...
async def deploy(event, repo, ups_rem, ac_br, txt):
    ...
    try:
        ...
        LOGS.error(e)
    try:
        add_to_collectionlist("restart_update", [IS0S0I.chat_id, IS0S0I.id])
    except Exception as e:
        LOGS.error(e)
    ups_rem.fetch(ac_br)
    repo.git.reset("--hard", "FETCH_HEAD")
    ...
    try:
        remote.push(refspec="HEAD:refs/heads/BRUS", force=True)
...
async def upstream(event):
    ...
    try:
        ...
        origin.fetch()
        force_update = True
        repo.create_head("BRUS", origin.refs.BRUS)
        repo.heads.BRUS.set_tracking_branch(origin.refs.BRUS)
        repo.heads.BRUS.checkout(True)
...
async def Hussein(event):
    ...
    off_repo = "https://github.com/almul8ab/BatIQ"
    os.chdir("/app")
    try:
        txt = (
        ...
        origin = repo.create_remote("upstream", off_repo)
        origin.fetch()
        repo.create_head("BRUS", origin.refs.master)
        repo.heads.BRUS.set_tracking_branch(origin.refs.master)
        repo.heads.BRUS.checkout(True)
