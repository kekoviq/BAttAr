...
@kekoviq.on(admin_cmd(pattern="(نشر_تلقائي|النشر_التلقائي)"))
async def _(event):
    ...
    if jok.startswith("@"):
        ...
        BRUS = jok.replace("https://t.me/", "@")
    elif str(jok).startswith("-100"):
        BRUS = str(jok).replace("-100", "")
    else:
        try:
            BRUS = int(jok)
...
async def _(event):
    ...
    for chat in channels_set:
        if event.media:
            ...
            await kekoviq.send_message(int(chat), event.message)
