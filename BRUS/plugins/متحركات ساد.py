...
async def tmgif(event):
    if event.fwd_from:
        return
    IS0S0I = await reply_id(event)
    if sad:
        kekoviq = f"**˛ 𝘼𝙇𝙨𝙖𝙝𝙚𝙧 . .**\n"
        kekoviq += f"✛━━━━━━━━━━━━━✛\n"
        kekoviq += f"**الـمتحـرڪـة الأولـى **"
        await event.client.send_file(event.chat_id, sad, caption=kekoviq, reply_to=IS0S0I)
