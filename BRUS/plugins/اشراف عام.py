...
from BRUS import CMD_HELP, kekoviq
...
async def get_full_user(event):  
    ...
    if event.reply_to_msg_id and not len(args) == 2:
        ...
        try:
            ...
            return await event.edit("▾∮ هنالك خطأ يرجى تبليغنا @ISVSVI", str(err))           
...
async def get_user_from_id(user, event):
    ...
    try:
        ...
        return None
    return user_obj
@kekoviq.on(admin_cmd(pattern="{up_admin} ?(.*)"))
async def gben(BRUS):
    dc = razan = BRUS
    i = 0
    sender = await dc.get_sender()
    me = await BRUS.client.get_me()
    ...
    if BRUS.is_private:
        user = BRUS.chat
        rank = BRUS.pattern_match.group(1)
    else:
        BRUS.chat.title
    try:
        user, rank = await get_full_user(BRUS)
    except:
        pass
    if me == user:
       kekoviq = await razan.edit("▾∮ لا استطيع رفع نفسي ،")
       return
    try:
        if not rank:
            rank = "ㅤㅤ"
    ...
    if user:
        telchanel = [d.entity.id
                     for d in await BRUS.client.get_dialogs()
        ...
        for x in telchanel:
          try:
             await BRUS.client(EditAdminRequest(x, user, rgt, rank))
             i += 1
...
@kekoviq.on(admin_cmd(pattern="{down_admin} ?(.*)"))
async def gben(BRUS):
    dc = razan = BRUS
    i = 0
    sender = await dc.get_sender()
    ...
    try:
        ...
        pass
    if me == user:
       kekoviq = await razan.edit("▾∮ لا استطيع تنزيل نفسي 🧸🤍")
       return
    try:
        if not rank:
            rank = "ㅤㅤ"
