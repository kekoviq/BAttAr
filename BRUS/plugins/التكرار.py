...
async def spam_function(event, BRUS, kekoviq, sleeptimem, sleeptimet, DelaySpam=False):

    counter = int(kekoviq[0])
    if len(kekoviq) == 2:
        spam_message = str(kekoviq[1])
        for _ in range(counter):
            if gvarstatus("spamwork") is None:
                return
            if event.reply_to_msg_id:
                await BRUS.reply(spam_message)
            else:
                await event.client.send_message(event.chat_id, spam_message)
            await asyncio.sleep(sleeptimet)
    elif event.reply_to_msg_id and BRUS.media:
        for _ in range(counter):
            if gvarstatus("spamwork") is None:
                return
            BRUS = await event.client.send_file(
                event.chat_id, BRUS, caption=BRUS.text
            )
            await _catutils.unsavegif(event, BRUS)
        ...
        if BOTLOG:
            ...
            BRUS = await event.client.send_file(BOTLOG_CHATID, BRUS)
            await _catutils.unsavegif(event, BRUS)
        return
    elif event.reply_to_msg_id and BRUS.text:
        spam_message = BRUS.text
        for _ in range(counter):
            if gvarstatus("spamwork") is None:
                return
            await event.client.send_message(event.chat_id, spam_message)
...
@kekoviq.ar_cmd(pattern="كرر (.*)")
async def spammer(event):
    BRUS = await event.get_reply_message()
    kekoviq = ("".join(event.text.split(maxsplit=1)[1:])).split(" ", 1)
    try:
        counter = int(kekoviq[0])
    except Exception:
    ...
    await spam_function(event, BRUS, kekoviq, sleeptimem, sleeptimet)

@kekoviq.on(admin_cmd(pattern=f"{Mukrr}"))
async def spammer(event):
    ...
    await event.delete()
    addgvar("spamwork", True)
    await spam_function(event, reply, kekoviq, sleeptimem, sleeptimet, DelaySpam=True)


@kekoviq.ar_cmd(pattern="تكرار الملصق$")
