...
DEFAULTUSERBIO = (
    str(DEFAULT_BIO)
    if DEFAULT_BIO
    else "كنَّا نموت إذا افترقنا ساعةً ‏واليوم نُحصي الهجر بالأعوام @kekoviqX"
)

@kekoviq.ar_cmd(pattern="انتحال(?:\s|$)([\s\S]*)")
async def _(event):
    mid = await kekoviq.get_me()
...
jeps = ["BRUS", "Qruesupport"]
@kekoviq.ar_cmd(pattern="انتحال_الدردشه")
async def reda(event):
    if event.is_group or event.is_channel:
        ...
        chat_id = msg
        try:
            result = await kekoviq(GetFullChannelRequest(
                chat_id
            ))
        except ValueError:
        ...
        mych = await kekoviq(GetFullChannelRequest(
                event.chat_id
            ))
        if msg in jeps:
        ...
        try:
            await kekoviq(functions.channels.EditTitleRequest(
                channel=await kekoviq.get_entity(event.chat_id),
        ...
        try:
            await kekoviq(functions.messages.EditChatAboutRequest(
            peer=event.chat_id,
            about=result.full_chat.about
        ))
        except FloodWaitError:
            return await edit_delete(event, "**انتضر مدة لا تقل عن 5 دقائق للانتحال مجدداً FLOODWAITERROR خطأ من التيليجرام**")
        profile_pic = await kekoviq.download_profile_photo(chat_id, Config.TEMP_DIR)
        pfile = await kekoviq.upload_file(profile_pic)
        try:
            await kekoviq(functions.channels.EditPhotoRequest(event.chat_id, pfile))
        except FloodWaitError:
            return await edit_delete(event, "**انتضر مدة لا تقل عن 5 دقائق للانتحال مجدداً FLOODWAITERROR خطأ من التيليجرام**")
...
#Reda
@kekoviq.ar_cmd(pattern="اعادة_الدردشه")
async def reda_back(event):
    if event.is_group or event.is_channel:
        if gvarstatus (f"{event.chat_id}name"):
            ...
            async for photo in kekoviq.iter_profile_photos(event.chat_id, limit=1) :
                    await kekoviq(
