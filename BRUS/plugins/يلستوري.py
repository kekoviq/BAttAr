...
@kekoviq.on(events.NewMessage(pattern=r".فشر(?: |$)(.*)"))
async def fsh(event):
...
@kekoviq.on(events.NewMessage(pattern=r".خلص"))
...
@kekoviq.on(events.NewMessage(pattern=r"\.تواجدي$"))
...
@kekoviq.on(events.NewMessage(pattern=r".غياب (\d+)([smhd])(?: |$)(.*)"))
...
@kekoviq.on(events.NewMessage(pattern=r".رجعت"))
...
async def sticker_to_image(event):
    ...
    await kekoviq.send_file(event.chat_id, image_path, force_document=False)
