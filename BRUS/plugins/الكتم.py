...
@kekoviq.ar_cmd(pattern=f"كتم(?:\s|$)([\s\S]*)")
...
@kekoviq.ar_cmd(pattern=f"(الغاء الكتم|الغاء كتم)(?:\s|$)([\s\S]*)")
...
async def handle_message(event):
    ...
    if private_mode.get(chat_id, False) and sender_id != (await kekoviq.get_me()).id:
        try:
            await event.delete()
