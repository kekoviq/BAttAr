...
from BRUS import CMD_HELP, LOAD_PLUG
...
def load_module(shortname, plugin_path=None):
    if shortname.startswith("__"):
        ...
        path = Path(f"BRUS/plugins/{shortname}.py")
        checkplugins(path)
        name = "BRUS.plugins.{}".format(shortname)
        ...
        spec.loader.exec_module(mod)
        # for imports
        sys.modules["BRUS.plugins." + shortname] = mod
        LOGS.info("᯽︙تم بنجاح تحميل ملف ✓" + shortname)
...
def remove_plugin(shortname):
        ...
        for cmdname in cmd:
            if cmdname in LOADED_CMDS:
                for i in LOADED_CMDS[cmdname]:
                    kekoviq.remove_event_handler(i)
    ...
    try:
        name = f"BRUS.plugins.{shortname}"
        for i in reversed(range(len(kekoviq._event_builders))):
            ev, cb = kekoviq._event_builders[i]
            if cb.__module__ == name:
                del kekoviq._event_builders[i]
    except BaseException:
        raise ValueError
...
def start_assistant(shortname):
    if shortname.startswith("__"):
        ...
        path = Path(f"BRUS/plugins/assistant/{shortname}.py")
        name = "BRUS.plugins.assistant.{}".format(shortname)
        spec = importlib.util.spec_from_file_location(name, path)
        mod = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(mod)
        ...
        mod.tgbot = bot.tgbot
        spec.loader.exec_module(mod)
        sys.modules["BRUS.plugins.assistant" + shortname] = mod
