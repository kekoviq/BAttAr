from BRUS import bot, kekoviq
#By Source saher @ISVSVI
...
'''
mm = '''
قم بلأنضمام الى قناة الخفاش @ISVSVI
'''
...
keyboard = [
    ...
    [
        Button.url("سورس الخفاش✨", "https://t.me/ISVSVI")
    ]
]
...
async def repo(event):
    ...
    response = await bot.inline_query(IS0S0I, "هاك")
...
async def start(event):
  ...
  if event.sender_id == bot.uid:
    ...
    [
        ...
        Button.url("المطور", "https://t.me/ISVSVI")
    ]
]
...
async def users(event):
  async with bot.conversation(event.chat_id) as x:
      ...
      if i == True:
          ...
          await event.reply(f"حدث خطأ قم بتوجيه الرسالة للمطور @IS0S0I\n{i}")
...
async def users(event):
  async with bot.conversation(event.chat_id) as x:
      ...
      try:
        ...
        if changing:
          await event.respond(" @IS0S0I لقد تم تغير الرقم بنجاح ✅")
        else:
...
async def start(event):
    keyboard = [
      ...
      [
        Button.url("القناة", "https://t.me/ISVSVI")
        ]
    ]
