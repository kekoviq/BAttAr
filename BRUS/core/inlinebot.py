...
def main_menu():
    text = f"**- [سـورس الخفاش](https://t.me/ISVSVI)\
        \n المسـاعد\
        \n\
        \n◽ الخفاش لـ {mention}**"
    buttons = [
...
async def inline_handler(event):  # sourcery no-metrics
    ...
    if query_user_id == Config.OWNER_ID or query_user_id in Config.SUDO_USERS:
        ...
        if query.startswith("kekoviq"):
            buttons = [
                (
                    Button.inline("الحـالة ⚒️", data="stats"),
                    Button.url(" BRUS UsᴇʀBoᴛ", "https://t.me/ISVSVI"),
                )
            ]
            ...
            builder = event.builder
            troll = os.path.join("./BRUS", "troll.txt")
            try:
            ...
            result = builder.article(
                title="رسـالة تـرول من سـورس الخفاش 🧸♥",
            ...
            user, txct = query.split(" ", 1)
            builder = event.builder
            secret = os.path.join("./BRUS", "secrets.txt")
            try:
            ...
            result = builder.article(
                title="همسـة سـرية من سـورس الخفاش 🧸♥",
                text=f"فقـط {razan} هـو مـن يستـطيع مشـاهدتهـا !",
                buttons=buttons,
            )
            await event.answer([result] if result else None)
            ...
            query = query[5:]
            builder = event.builder
            hide = os.path.join("./BRUS", "hide.txt")
            try:
            ...
            result = builder.article(
                title="رسـالة مخفيـة من سـورس الخفاش 🧸♥",
                text=f"✖✖✖",
                buttons=buttons,
            )
            await event.answer([result] if result else None)
            ...
            result = builder.article(
                title="© BRUS Help",
                description="Help menu for BRUS",
                text=_result[0],
                buttons=_result[1],
                link_preview=False,
            )
        ...
        buttons = [
            (
                Button.url("قنـاة السـورس ⚒️", "https://t.me/ISVSVI"),
                Button.url(
                    "كـروب المـساعدة 📬",
                    "https://t.me/ISVSVI",
                ),
        ...
        text, msg_entities = await event.client._parse_message_text(
            "**[بـوت الخفاش 🧸♥](https://t.me/ISVSVI)**\
            \n\
            \n❤ الخفاش هـو بـوت بسـيط يدخـل الـى حسـابك لجعـلك تتحـكم به معـ اوامـر معينـة.\
            \n\
...
@kekoviq.tgbot.on(CallbackQuery(data=re.compile(b"close")))
@check_owner
...
@kekoviq.tgbot.on(CallbackQuery(data=re.compile(b"check")))
...
@kekoviq.tgbot.on(CallbackQuery(data=re.compile(b"(.*)_menu")))
@check_owner
async def on_plug_in_callback_query_handler(event):
...
@kekoviq.tgbot.on(CallbackQuery(data=re.compile(rb"mainmenu")))
@check_owner
async def on_plug_in_callback_query_handler(event):
    _result = main_menu()
